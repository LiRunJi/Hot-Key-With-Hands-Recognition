...
class My_indentify():
    def __init__(self):
        ...
        self.ges_dict = np.load(resource_path('DataForTraining/all_gestures.npy'),allow_pickle=True).tolist()

        print(self.ges_dict)
        self.my_model = load_model(resource_path('TrainingOutput/ModelFiles'))  # 加载训练好的tensorflow模型
        ...
        while True:
            # if MyGlobalStates.__run__ is True:
                ...
                if success:
                    ...
                    # 显示我们预测的手势与概率
                        if show_img:
                            cv2.putText(image, str(self.now_ges), (100, 200), 0, 1.3, (0, 0, 255), 3)
                            # print(results.multi_handedness)
                            cv2.putText(image, str(np.max(pre)), (0, 100), 0, 1.3, (0, 0, 255), 3)
                            # 将手部的特征点连线显示
